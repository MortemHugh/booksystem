...
router.get('/', async(req, res)=>{
    try {
        ...
        const getRoomTypesQuery = `
            ...
            WHERE hotelid = $1
        `
        const roomType = await pool.query(getRoomTypesQuery, [hotelid])
        ...
        const roomsResult = await pool.query(
            'SELECT * FROM rooms r LEFT JOIN reservations rs ON r.roomid = rs.roomid JOIN room_type rt ON r.typeid = rt.typeid WHERE r.hotelid = $1 AND r.status != $2 ORDER BY r.roomnum ASC',
            [hotelid, 'Out-of-Order']
        )
        ...
        const floorResult = await pool.query(
            'SELECT DISTINCT roomfloor FROM rooms WHERE hotelid = $1 AND status != $2 ORDER BY roomfloor ASC',
            [hotelid, 'Out-of-Order']
        )
...
router.post('/book', async (req, res) => {
    ...
    try {
        ...
        const assignedRoomResult = await pool.query(
            ...
            ORDER BY RANDOM()
            LIMIT 1`,
            [roomtype, hotelid, 'Out-of-Order', checkindate, checkoutdate, roomfloor]
        )
        ...
        const htmlContent = `
            ...
            * {
            ...
            <p style="font-size: 14px; line-height: 170%;"><strong>Room Type:</strong> ${roomtypeee}<br /><strong>Room Number:</strong> ${roomnum}<br /><strong>Number of Days:</strong> ${numofdays}<br /><strong>Number of Adults:</strong> ${adultno}<br /><strong>Number of Children:</strong> ${childno}<br /><strong>Promo Code:</strong> ${promocode}</p>
            </div>
